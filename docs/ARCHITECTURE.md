...
| 진입점 | 내용 | 쓰는 쪽 |
|---|---|---|
| `@lounge/core` | 타입, 토큰, 검증, frontmatter. node 내장 모듈 없음 | 웹뷰 번들 포함 누구나 |
| `@lounge/core/fs` | 탐색, 읽기, 쓰기, ref 지문 | mcp, viewer(읽기만) |
...
앱은 `chokidar` 로 `.lounge/` 를 감시한다. 파일이 바뀌면 화면이 갱신된다.
폴링도 소켓도 없다.
...
core    zod, yaml 뿐. 순수 계층은 node 내장 모듈도 안 씀
mcp     @modelcontextprotocol/sdk (stdio)
viewer  Tauri + React, chokidar
```
